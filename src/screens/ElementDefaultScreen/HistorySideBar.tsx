...
import { ImageCompressor, storageManager } from "./storageUtils";
        ...
        <div className="clear-history-overlay">
            <div className="clear-overlay-body">
                <h4 className="clear-overlay-warning-title">
                    This will permanently delete your image generation history
                    from this device.
                </h4>

                <p className="clear-overlay-description">
                    Your image generation history is stored locally on your
                    device using browser local storage. Clearing this will
                    permanently delete all image generation history from this
                    device, but won't affect the history stored on other
                    devices. Your image generation history is not stored on our
                    servers.
                </p>
            </div>
            ...
            <div className="clear-overlay-footer">
                ...
                <button
                    className="clear-overlay-confirm-btn"
                    onClick={onConfirm}
                >
                    Clear history
                </button>
...
const HistorySidebar: React.FC<HistorySidebarProps> = ({
    ...
    const [hasUnselectedItems, setHasUnselectedItems] =
            ...
            thumbnailBlobUrls.forEach((url) => {
                try {
                    if (url && url.startsWith("blob:")) {
                        ...
                        console.log(
                            "🧹 Revoked thumbnail blob URL during cleanup"
                        );
                    }
                } catch (e) {
    ...
    const handleItemClick = async (item: HistoryItem) => {
        ...
        try {
            ...
            const sessionImages = await storageManager.getSessionImages(
                item.id
            );
            ...
            if (sessionImages && sessionImages.length > 0) {
                    ...
                    sessionImages.map(async (img) => {
                        try {
                            ...
                            if (
                                ...
                                console.log(
                                    "🔄 Converting base64 to blob for history item"
                                );
                                const compressed =
                            ...
                            console.warn(
                                "⚠️ Failed to convert to blob, using original:",
                                error
                            );
                            return {
                                imageBase64: img.imageUrl,
                ...
                console.log("✅ Converted images for history item:", {
                    ...
                    blobCount: convertedImages.filter((img) => img.isBlob)
                        .length,
                });
    ...
    return (
        <>
            {historyData.map((dateGroup, groupIndex) => (
                <div
                    key={`date-${groupIndex}-${dateGroup.date}`}
                        ...
                        {dateGroup.items.map((item) => {
                            ...
                            return (
                                <div
                                    ...
                                    onClick={() =>
                                        !isItemDisabled && handleItemClick(item)
                                    }
                                    style={{
